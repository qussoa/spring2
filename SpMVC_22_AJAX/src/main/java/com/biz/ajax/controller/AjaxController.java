...
public class AjaxController {
	...
	/*
	 * @ResponseBody 컨트롤러 method에서 return 하는 값을 변환없이 있는 그대로 보내라
	 * 
	 * 일반 method에서는 문자열에 해당하는 view를 찾아서 rendering을 수행하여 Client에게 보낸다
	 */
	@ResponseBody
	 ...
	 *  업무를 처리하고
	 *  그렇지 않은 요청은 모두 무시하게 되는 패턴
	 *  
	 *  이 패턴을 사용하여 web서비스를 만들려면
	 *  프로젝트의 web.xml의 appServlet url-pattern을 설정하고
	 *  대신 index.html, index.jsp 파일을 webapp 폴더에 작성해 두어야한다
	 *  그래야 프로젝트가 톰켓에 의해 run되었을떄
	 *  최초 404 오류가 나타나지 않는다
	 */
	...
	/*
	 * @ResponsBody를 붙이고 객체(클래스)를 리턴하면
	 * 객체 json으로 변환되어 클라이언트에 전송
	 * 단, jacksonbind를 pom.xml에 dependency를 설정해 두어야한다
	 */
	@ResponseBody
	 ...
	 * 문자열을 return하고 해당하는 문자열.jsp파일을 views 폴더에서
