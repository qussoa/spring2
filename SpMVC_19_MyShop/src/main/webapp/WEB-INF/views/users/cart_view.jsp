...
<script>
	$(function() {
		...
		$(".btn_one_delete")
				.click(
						function() {
							...
							if (confirm("상품 취소")) {
								document.location
										.replace("${rootPath}/user/product/cart_one_delete/"
												+ id)
							}
		...
		$(".btn_list_delete").click(function() {
			...
			// 본문에 있는 cart_list_check를 뽑아서 배열 만들기
			let checkList = $(".cart_list_check")
			// 배열을 순회하면서 
			// checked가 되어있는 check box만 찾아서 
		...
		$(".btn_list_buy").click(function() {
			...
			if(confirm(cart_array.length + "개의 상품 주문")){
				$.ajax({
...
<c:choose>
	...
	<c:otherwise>
		<form id="cart_form" method="POST"
			action="${rootPath}/user/product/cart_list_qty_update">
				...
				<div class="cart-list" data-id="${cart.seq}">
					...
					<p>
						<label>수량</label> <input type="number" name="p_qty"
							value="${cart.p_qty}" id="p_${cart.seq}" /> <input type="hidden"
							name="seq" value="${cart.seq}">
...
<div>
	...
	<button class="btn_list_delete">주문취소</button>
