...
public interface CartDao {
	...
	@Select("SELECT P.p_code, P.p_name, C.username, C.p_oprice, C.p_qty"
			+ "  FROM tbl_cart C LEFT JOIN tbl_product P ON C.p_code = P.p_code"
	...
	// 주문 완료 후 배송중 SELECT
	@Select("SELECT * FROM tbl_cart WHERE username = #{username} AND p_status = 'DELI'")
	public List<CartVO> selectDelivery(@Param("username") String username);

	// 관리자가 현재 배송중인 상품이 몇건이나 있는 조회할 때 
	@Select("SELECT COUNT(*) FROM tbl_cart WHERE p_status = 'DELI'")
