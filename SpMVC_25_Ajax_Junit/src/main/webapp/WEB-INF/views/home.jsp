...
<script>
	/*
		...
		기능이 제대로 작동되지 않는다 
		jquery에서 $(function(){코드들})형식으로 작성을 하면
		...
		<form action="saveUser" method="POST">
			...
			<%
				/*
					...
					이 버튼이 form 안에 있으면
					버튼을 클릭했을때 form안의 input box에 입력한 값을 모두 모아서
					action으로 지정된 url로 모두 전송하는 기능을 수행
					
					button type="button"으로 지정하면 모양만 버튼이고 
					기본 기능을 수행하지 않도록 한다
				*/
			%>
			<button type="submit">저장</button>
			<button type="button" class="userId">사용자 Id 전송</button>
			<button type="button" class="user">입력 값 전송</button>
			<button type="button" class="user_html">입력 값 html로 받기</button>
		</form>
	</section>
</body>
</html>