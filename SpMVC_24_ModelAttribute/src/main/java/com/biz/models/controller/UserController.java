...
public class UserController {
	 ...
	 * Qualifier(객체 이름 지정하기)를 지정해주면 이 method는
	 * 컨트롤러의 다른 method가 실행되기전에 코드를 실행하여
	 * 객체를 model에 담아두는 일을 수행한다
	 * 
	 * 컨트롤러에서 
	 * view에 rendering을 위한 데이터를 내려보낼때
	 * 필요할 때마다 model을 생성하고 model에 addAttribute()를 실행해서
	 * 데이터를 보내는데, 공통적으로 view에서 사용해야할 데이터가 있을때
	 * 
